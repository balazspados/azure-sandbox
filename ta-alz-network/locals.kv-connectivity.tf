locals {
  key_vault_connectivity_parameters = {
    ...
    kv_name = "${local.alz_config.org_id}-kv-con-${local.alz_config.environment}-${local.alz_config.azure_region_location_short}-001"
    ...
    private_endpoints_subnet_id                  = module.platform_vnet_005.subnets.private_endpoints.resource_id
    private_endpoints_subnet_resource_group_name = azurerm_resource_group.rg_nw_005.name
    private_dns_zone_id                          = module.private_dns_zones.private_dns_zone_resource_ids["azure_key_vault"]
